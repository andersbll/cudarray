...
def _max_pool_seg_b01(ArrayData imgs, int n_frag, img_shape, int n_chan, win_shape, padding,
    ...
    cdef int stride_x = strides[1]
    if is_float(imgs):
        nsnet.max_pool_seg_b01(float_ptr(imgs), n_frag, img_h, img_w,
            n_chan, win_h, win_w, pad_y, pad_x, stride_y, stride_x,
            float_ptr(out), int_ptr(mask))
    else:
        raise ValueError('type %s not implemented' % str(imgs.dtype))


def _max_pool_seg_b01_bprop(ArrayData out_d, ArrayData mask, int n_frag, img_shape,
    int n_chan, win_shape, padding, strides, ArrayData imgs_d):
    cdef int img_h = img_shape[0]
    cdef int img_w = img_shape[1]
    cdef int win_h = win_shape[0]
    cdef int win_w = win_shape[1]
    ...
    cdef int stride_x = strides[1]
    if is_float(out_d):
        nsnet.max_pool_seg_b01_bprob(float_ptr(out_d), int_ptr(mask),
            n_frag, img_h, img_w, n_chan, win_h, win_w, pad_y, pad_x, stride_y,
